var UserDeletionMigration = function (_, q, DB_SERVER, DB_PORT, FORM_DB_NAME, DRISTHI_DB_NAME, USER_NAME) {
    ...
    var createActionView = function () {
        return dristhiRepository.createView('_design/Action_Temp', {
                byANMId: {
                    map: function (doc) {
                        if (doc.type == 'Action') {
                            emit(doc.anmIdentifier);
    ...
    var getActionForTheUser= function () {
        var deferred = q.defer();
        dristhiRepository
            .queryView('Action_Temp/byANMId', {
                include_docs: true,
                key: userName
            })
            .then(function (response) {
    ...
    var deleteActionDocuments = function () {
        var deferred = q.defer();
        var actionDocument = _.map(actions, function (action) {
            return action.doc;
        });
        dristhiRepository.deleteDocs(actionDocument);
        deferred.resolve();
        return deferred.promise;
    };
    ...
    var deleteAction_TempView = function () {
        return dristhiRepository.deleteDocById('_design/Action_Temp');
    };

    var cleanupViewsAndCompactDB = function () {
        return q.all([deleteFormSubmissionsForAnUser_TempView(), deleteEntitiesForAnUser_TempView(), deleteDristhiUser_TempView(), deleteAction_TempView()]);
    };
