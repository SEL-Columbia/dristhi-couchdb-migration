var ANCMigration = function (_, q, DB_SERVER, DB_PORT, FORM_DB_NAME, DRISTHI_DB_NAME) {
    ...
    var getAllIFATabletsForANC = function () {
        var deferred = q.defer();
        var entityIds = _.pluck(openANCs, 'value');
        formRepository
            .queryView('FormSubmission_Temp/byFormNameAndEntityId',
    ...
    var getAllTTDosesForANC = function () {
        var deferred = q.defer();
        var entityIds = _.pluck(openANCs, 'value');
        var keysForTT = [];
    ...
    var updateANCWithIFATabletsInformation = function () {
        var deferred = q.defer();
        var ifaTablets = _.map(ifaFormSubmissions, function (r) {
            var fields = _.filter(r.doc.formInstance.form.fields, function (field) {
        ...
        _.each(openANCs, function (anc) {
            var allIFAsForMother = _.where(ifaTablets, {entityId: anc.value});
            _.each(allIFAsForMother, function (ifa) {
                delete ifa.entityId;
            });
            anc.key.ifaTablets = allIFAsForMother;
    ...
    var updateANCWithTTDosesInformation = function () {
        var deferred = q.defer();
        ...
        _.each(openANCs, function (anc) {
            ...
            anc.key.ttDoses = allTTsForMother;
    ...
    var migrate = function () {
        connectToDB()
            ...
            .then(getAllIFATabletsForANC)
            .then(updateANCWithIFATabletsInformation)
            .then(getAllTTDosesForANC)
            .then(updateANCWithTTDosesInformation)
